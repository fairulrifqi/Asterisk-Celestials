...
# Anda bisa mengubah 'model_predictions.log' ke path yang sesuai di lingkungan deployment.
# Level logging diatur ke INFO, artinya semua pesan INFO, WARNING, ERROR, dan CRITICAL akan dicatat.
logging.basicConfig(
    filename='model_predictions.log',
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)
# Logger untuk aplikasi Streamlit
...
# Inisialisasi session state untuk metrik jika belum ada
if 'total_predictions' not in st.session_state:
    st.session_state.total_predictions = 0
if 'last_prediction_latency' not in st.session_state:
    st.session_state.last_prediction_latency = 0.0

# Tombol untuk memulai proses prediksi
if st.button("Submit & Prediksi", type="primary"):
    start_time = time.time()
    st.write("Memulai proses prediksi...")
    ...
    if all([general_data_file, employee_survey_file, manager_survey_file, in_time_file, out_time_file]):
        ...
        app_logger.info(
            f"File diunggah: General Data: {general_data_file.name}, "
            f"Employee Survey: {employee_survey_file.name}, "
            f"Manager Survey: {manager_survey_file.name}, "
            f"In-Time: {in_time_file.name}, "
            f"Out-Time: {out_time_file.name}"
        )
        ...
        with st.spinner("Sedang memproses data dan menjalankan prediksi..."):
            try:
                ...
                # Pastikan file 'tuned_classifier.pkl' ada di direktori yang sama
                # dengan aplikasi Streamlit Anda saat di-deploy.
                ...
                # Hitung dan simpan latensi
                end_time = time.time()
                latency = end_time - start_time
                st.session_state.last_prediction_latency = latency
                st.session_state.total_predictions += 1 # Tambah hitungan prediksi sukses

                st.write(f"Waktu yang dibutuhkan untuk prediksi: {latency:.2f} detik")

                # Log hasil prediksi (opsional, bisa disesuaikan untuk logging data yang lebih ringkas)
                app_logger.info(
                    f"Prediksi berhasil. Jumlah record: {len(res)}, "
                    f"Waktu komputasi: {latency:.2f} detik. "
                    f"Contoh output: {res.head().to_dict('records')}"
                )
                ...
                app_logger.error(f"Terjadi kesalahan saat prediksi: {e}", exc_info=True)

    else:
        st.warning("⚠️ Mohon unggah kelima file yang diperlukan untuk melanjutkan.")
        app_logger.warning("Upaya prediksi tanpa semua file yang diunggah.")

st.divider()
st.subheader("Metrik Aplikasi")
st.info(f"Total Prediksi Berhasil: {st.session_state.total_predictions}")
st.info(f"Latensi Prediksi Terakhir: {st.session_state.last_prediction_latency:.2f} detik")